#+title: MakeFile Creation script
#+author: oddloops
#+PROPERTY: header-args:shell :tangle ./makeMake :shebang "#!/bin/bash"
* makeMake
A script to automate the creation of makefile for C lang use. Takes in three arguments:
1. Name of the compiler to use.
2. Name (without the file extension) of the C source.
3. Name for the makefile.
...
if [ "$#" -eq 3 ]; then
    ...
    echo -en "Enter a value for (3)Name (without extension) of C: "
    read ARG_3
elif [ "$#" -eq 1 ]; then 
    # prompt for second and third args
    ARG_1=$1
    echo -en "Enter a value for (2)Name (without extension) of C: "
    read ARG_2
...
echo "DEBUG_FLAGS= -g -c -Wall -std=c11" >> "$ARG_3"
...
echo >> "$ARG_3"
echo >> "$ARG_3"
echo "# Recipes for targets" >> "$ARG_3"
echo >> "$ARG_3"

for name in "${objects[@]}"; do
    [ -f "${name}.h" ] && echo "${name}.o: ${name}.c ${name}.h" >> "$ARG_3" || \
	    echo "${name}.o: ${name}.c" >> "$ARG_3"
